...
bunun için pointer e sona götüren hasNext() meth calışmalı
 */
        ListIterator<Integer>it1=l1.listIterator();
        while (it1.hasNext()){// bastan sona pointer sürekli sona doğru ilerler
            it1.next();
       ...
       // System.out.println("tersten print hali l1 = " + l1);



        List<String>l2=new ArrayList<>(Arrays.asList("cağla","ugur","muharrem","cebrail"));
        //l2 elemanlarının sonuncusuna  :-) ekleyip print ediniz
        System.out.println("listitarator öncesi l2 :"+l2);

        ListIterator<String>it2=l2.listIterator();//l2 elamanların tekrarlayan listiteretor tanımlandı
        while (it2.hasNext()){
